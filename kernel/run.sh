...
while read -r line; do
    ...
    case "$type" in
        ...
        p)
            objects+=($object)
            gcc -m32 -ffreestanding -nostdlib -nostartfiles -fno-ident -fno-asynchronous-unwind-tables -c "$file" -o "$object" || exit 1 ;;
...
ld -m elf_i386 -T "${location}/ld" --entry=_start "${objects[@]}" -o "$elf" || exit 1
