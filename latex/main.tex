...
% To enable paging, use MOV to set CR0.PG in CR0, before doing so CR3 should contain the physical address of the first paging structure

% 5.1 - paging modes + control bits
% If CR0.PG = 0 then paging is not used and all linear addresses are treated by the processor as physical addresses
% if CR0.PG = 1 then paging can only be enabled if protection also is (CR0.PE = 1), one of four paging modes are used which depend on the values of CR4.PAE, CR4.LA57 and IA32\_EFER.LME

% CR4.PAE = 0
% 32-bit paging is used.
% processor ensures IA32\_EFER.LME = 0
%
% CR4.PAE = 1 and IA32\_EFER.LME = 0
% PAE paging is used
%
% CR4.PAE = 1 and IA32\_EFER.LME = 1 and CR4.LA57 = 0
% 4-level paging is used
%
% CR4.PAE = 1 and IA32\_EFER.LME = 1 and CR4.LA57 = 1
% 5-level paging is used

% 32-bit and PAE can only be used in legacy protected mode (not long mode)
% In contrast, 4-level and 5-level can only be used in IA-32e mode (long mode)
